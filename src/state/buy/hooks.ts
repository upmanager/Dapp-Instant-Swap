...
import { Field, replaceBuyState, selectCurrency, setRecipient, switchCurrencies, typeInput } from './actions'
import { BuyState } from './reducer'

export function useBuyState(): AppState['buy'] {
  return useAppSelector((state) => state.buy)
}

export function useBuyActionHandlers(): {
    ...
    (field: Field, currency: Currency) => {
      if (field === Field.OUTPUT) {
        ...
        dispatch(
          selectCurrency({
            field,
            currencyId: newOutputCurrencyId,
          })
        )
      }
    },
...
export function queryParametersToSwapState(parsedQs: ParsedQs, chainId: ChainId = ChainId.ETHEREUM): BuyState {
...
export function useDefaultsFromURLSearch():
  ...
  useEffect(() => {
    ...
    router.replace(
      `${router.pathname}?inputCurrency=${parsed[Field.INPUT].currencyId}&outputCurrency=${
